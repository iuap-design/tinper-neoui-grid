...
(function ($, window, document, undefined) {
  ...
  */
	...
	var gridCompColumn = function gridCompColumn(options, gridComp) {
		this.init(options, gridComp);
	};
  ...
  */
   ...
   */
		...
		getBooleanOptions: function getBooleanOptions() {
   ...
   */
		...
		initOptionsTree: function initOptionsTree() {},
		/*
   * 初始化变量
   */
   ...
   * 初始化datasource相关变量
   */
		initDataSourceVariable: function initDataSourceVariable() {
		...
		// 初始化宽度相关变量
		initWidthVariable: function initWidthVariable() {
			// 计算用变量
   ...
   */
		initGridCompColumn: function initGridCompColumn() {
			var oThis = this;
		...
		initGridCompColumnVar: function initGridCompColumnVar() {
		...
		initGridCompColumnFun: function initGridCompColumnFun(columnOptions) {
		...
		initGridCompColumnColumnMenuFun: function initGridCompColumnColumnMenuFun(columnOptions) {},
		initGridCompColumnHeaderLevelFun: function initGridCompColumnHeaderLevelFun(columnOptions) {},
		initGridCompColumnLoacl: function initGridCompColumnLoacl(columnOptions) {},
		initGridHiddenLevelColumn: function initGridHiddenLevelColumn() {},
		initGridCompFixedColumn: function initGridCompFixedColumn() {},
		/*
   * 设置某列是否必输
   */
		setRequired: function setRequired(field, value) {},
		/*
   ...
   * 创建div区域
   */
		repaintDivs: function repaintDivs() {
			// 后期可以考虑form展示
   ...
   * 创建grid形式下div区域
   */
		createGridDivs: function createGridDivs() {
   ...
   * 重画grid
   */
		repaintGridDivs: function repaintGridDivs() {
   ...
   * 创建columnMenu区域
   */
		createColumnMenu: function createColumnMenu() {
			return '';
		},
		/*
   * 创建header区域
   ...
   * 创建header区域table
   */
		createHeaderTable: function createHeaderTable(createFlag) {
		...
		createHeaderTableFixed: function createHeaderTableFixed() {
			return '';
		},
		createHeaderDrag: function createHeaderDrag() {
			return '';
		},
		/*
   * 创建colgroup
   */
		createColgroup: function createColgroup(createFlag) {
			var oThis = this,
			    htmlStr = '<colgroup>',
			    gridCompColumnArr;
			if (createFlag == 'fixed') {
   ...
   * 创建thead区域
   */
		createThead: function createThead(createFlag) {
			var oThis = this,
			    visibleIndex = 0,
			    gridCompColumnArr,
			    trStyle = '';
     ...
     */
		...
		createContentSumRow: function createContentSumRow() {
			return '';
		},
		/*
   * 创建内容区左侧区域
   */
		createContentLeft: function createContentLeft() {
			var oThis = this,
			    htmlStr = "",
			    left = 0,
			    hStr;
   ...
   * 创建内容区左侧区域复选区（一行）
   */
		createContentLeftMultiSelectRow: function createContentLeftMultiSelectRow(row, displayFlag) {
			var displayStr = '';
   ...
   * 创建内容区左侧区域数字列（一行）
   */
		createContentLeftNumColRow: function createContentLeftNumColRow(index) {
   ...
   * 创建内容区table
   */
		createContentTable: function createContentTable(createFlag) {
		...
		createContentTableFixed: function createContentTableFixed() {
			return '';
		},
		/*
   * 创建无数据区域
   */
		createNoRowsDiv: function createNoRowsDiv() {
			var styleStr = '',
			    styleStr1 = '';
			if (this.contentMinWidth > 0) {
   ...
   * 创建内容区域所有行
   */
		createContentRows: function createContentRows(createFlag) {
			var oThis = this,
			    htmlStr = "",
			    idStr;
			if (createFlag == 'fixed') {
		...
		createContentRowsSumRow: function createContentRowsSumRow() {
			return '';
		},
		/*
   * 创建内容区域数据行
   */
		createContentOneRow: function createContentOneRow(row, createFlag, displayFlag) {
			var styleStr = '';
			if (!this.options.autoExpand && row.level > 0 && displayFlag != 'block') {
   ...
   * 创建内容区域数据行，针对IE
   */
		createContentOneRowForIE: function createContentOneRowForIE(table, index, rowObj, createFlag, displayFlag) {
   ...
   * 数据更新重画当前行
   */
		repaintRow: function repaintRow(rowIndex) {
   ...
   * 创建行td对应的html
   */
		createContentOneRowTd: function createContentOneRowTd(row, createFlag) {
			var oThis = this,
			    htmlStr = '',
			    gridCompColumnArr,
			    value = row.value;
   ...
   * 创建行td,针对IE
   */
		createContentOneRowTdForIE: function createContentOneRowTdForIE(row, rowObj, createFlag) {
			var oThis = this,
			    gridCompColumnArr,
			    value = rowObj.value;
			if (createFlag == 'fixed') {
   ...
   * 重画内容区域
   */
		repairContent: function repairContent() {
   ...
   * 创建完成之后顶层div添加监听
   */
		initEventFun: function initEventFun() {
			var oThis = this;
   ...
   * 创建完成之后grid层 div添加监听
   */
		initGridEventFun: function initGridEventFun() {
			var oThis = this;
			// 拖动
			this.initContentDivEventFun();
			// 全选
   ...
   * 内容区 div添加监听
   */
		initContentDivEventFun: function initContentDivEventFun() {
			var oThis = this;
			// 通过复选框设置选中行
		...
		trHoverFun: function trHoverFun(index) {
			var oThis = this;
			$('#' + oThis.options.id + '_content_tbody').find('tr').removeClass('u-grid-move-bg');
			$('#' + oThis.options.id + '_content_fixed_tbody').find('tr').removeClass('u-grid-move-bg');
			if (oThis.options.multiSelect) $('#' + oThis.options.id + '_content_multiSelect').find('div').removeClass('u-grid-move-bg');
   ...
   * 定时器处理
   */
		setIntervalFun: function setIntervalFun(e) {
			this.widthChangeFun();
			this.heightChangeFun();
			this.editorRowChangeFun();
		},
		editorRowChangeFun: function editorRowChangeFun() {},
		/*
   * grid区域创建完成之后处理
   * 1、数据列显示处理
   * 2、取行高
   */
		afterGridDivsCreate: function afterGridDivsCreate() {
			this.columnsVisibleFun();
   ...
   * 取行高
   */
		countRowHeight: function countRowHeight() {
   ...
   * 处理是否显示无数据行
   */
		noRowsShowFun: function noRowsShowFun() {
   ...
   * 更新最后数据行标识
   */
		updateLastRowFlag: function updateLastRowFlag() {
			// 共享服务加的，没有对应的css暂时去掉
			return;
		...
		updateNumColLastRowFlag: function updateNumColLastRowFlag() {
			// 共享服务加的，没有对应的css暂时去掉
			return;
   ...
   */
		renderTypeFun: function renderTypeFun(obj) {
   ...
   * begin为起始行，length为行数（增加行数时使用）
   */
		renderTypeByColumn: function renderTypeByColumn(gridCompColumn, i, begin, length, isFixedColumn) {
			var oThis = this,
		...
		renderTypeSumRow: function renderTypeSumRow(gridCompColumn, i, begin, length, isFixedColumn) {},
		/*
   * grid区域重画完成之后处理，已经执行过afterGridDivsCreate
   * 1、设置横向滚动条
   * 2、隐藏编辑按钮
   */
		afterRepaintGrid: function afterRepaintGrid() {
			this.resetScrollLeft();
			this.hideEditMenu();
		},
		/*
   * 设置横向滚动条
   */
		resetScrollLeft: function resetScrollLeft() {
			if ($('#' + this.options.id + '_content_div')[0]) {
				try {
					$('#' + this.options.id + '_content_div')[0].scrollLeft = this.scrollLeft;
				} catch (e) {}
   ...
   * 整体宽度改变处理
   */
		widthChangeFun: function widthChangeFun() {
			var oThis = this;
   ...
   * 整体宽度改变处理(grid形式)
   */
		widthChangeGridFun: function widthChangeGridFun() {
			var oThis = this,
		...
		widthChangeGridFunFixed: function widthChangeGridFunFixed(halfWholeWidth) {},
		widthChangeGridFunOverWidthHidden: function widthChangeGridFunOverWidthHidden() {},
		/*
   * 整体高度改变处理
   */
		heightChangeFun: function heightChangeFun() {
			if (this.countContentHeight) {
   ...
   * 计算内容区域最后一列显示列
   */
		columnsVisibleFun: function columnsVisibleFun() {
			var oThis = this,
			    w = 0;
   ...
   * 创建完成之后处理变量
   */
		resetThVariable: function resetThVariable() {
		...
		resetThVariableDrag: function resetThVariableDrag(nowTh, gridCompColumn) {},
		resetThVariableHeaderLevel: function resetThVariableHeaderLevel() {},
		/*
   * 内容区宽度改变
   */
		contentWidthChange: function contentWidthChange(newContentWidth) {
   ...
   * 获取某列对应属性
   */
		getColumnAttr: function getColumnAttr(attr, field) {
   ...
   * 获取column属于第几列
   */
		getIndexOfColumn: function getIndexOfColumn(column) {
			var index = -1;
			for (var i = 0; i < this.gridCompColumnArr.length; i++) {
   ...
   * 获取column属于当前显示第几列
   */
		getVisibleIndexOfColumn: function getVisibleIndexOfColumn(column) {
			var index = -1;
			var j = 0;
			for (var i = 0; i < this.gridCompColumnArr.length; i++) {
				if (this.gridCompColumnArr[i] == column) {
   ...
   */
		getNextVisibleInidexOfColumn: function getNextVisibleInidexOfColumn(column) {
			var index = -1,
			    flag = false,
			    j = 0;
			for (var i = 0; i < this.gridCompColumnArr.length; i++) {
   ...
   * 修改第一列的css
   */
		headerFirstClassFun: function headerFirstClassFun() {
   ...
   * 双击/单击处理
   */
		isDblEvent: function isDblEvent(eventname, dbFun, dbArg, Fun, Arg) {
   ...
   * 双击处理
   */
		dblClickFun: function dblClickFun(e) {
   ...
   */
		...
		clickFunTree: function clickFunTree(e) {},
		clickFunEdit: function clickFunEdit(e) {},
		/*
   * 设置某列是否显示(传入column)
   */
		setColumnVisibleByColumn: function setColumnVisibleByColumn(column, visible) {
   ...
   */
		setColumnVisibleByIndex: function setColumnVisibleByIndex(index, visible) {
			if (index >= 0) {
   ...
   * 根据field设置宽度
   */
		setCoulmnWidthByField: function setCoulmnWidthByField(field, newWidth) {
   ...
   * 根据column对象设置宽度
   */
		setColumnWidth: function setColumnWidth(column, newWidth) {
   ...
   * 对宽度和高度进行处理
   */
		formatWidth: function formatWidth(w) {
			// 获得宽度
			if (w) {
   ...
   */
		swapEle: function swapEle(e1, e2) {
			var n = e1.next(),
			    p = e2.prev();
			e2.insertBefore(n);
			e1.insertAfter(p);
		},
		getString: function getString(value, defaultValue) {
		...
		getInt: function getInt(value, defaultValue) {
			if (value === null || value === undefined || value === 'null' || value === 'undefined' || value === "" || isNaN(value)) {
				value = defaultValue;
			}
			return value;
		},
		getFloat: function getFloat(value, defaultValue) {
			if (value === null || value === undefined || value === 'null' || value === 'undefined' || value === "" || isNaN(value)) {
				value = defaultValue;
			}
			return value;
   ...
   * 处理精度
   */
		DicimalFormater: function DicimalFormater(obj) {
			var value = obj.value + '',
		...
		checkDicimalInvalid: function checkDicimalInvalid(value, precision) {
		...
		getTrIndex: function getTrIndex($tr) {
   ...
   */
		setDataSourceFun1: function setDataSourceFun1(dataSource) {
			var dataSourceObj = {};
   ...
   * 添加一行
   */
		addOneRow: function addOneRow(row, index) {
			var oThis = this,
			    displayFlag = 'none',
			    rowObj = {},
			    parentIndex,
		...
		addOneRowTree: function addOneRowTree(row, index) {
			return index;
		},
		addOneRowTreeHasChildF: function addOneRowTreeHasChildF() {},
		editClose: function editClose() {},
		/*
   * 添加多行
   */
		addRows: function addRows(rows, index) {
			if (this.options.showTree) {
				// 树表待优化
				var l = rows.length;
				for (var i = 0; i < l; i++) {
		...
		createContentOneRowFixed: function createContentOneRowFixed(rowObj) {
			return '';
		},
		updateEditRowIndex: function updateEditRowIndex(opType, opIndex, num) {},
		/*
   * 删除一行
   */
		deleteOneRow: function deleteOneRow(index) {
			var oThis = this;
		...
		repairSumRow: function repairSumRow() {},
		deleteOneRowTree: function deleteOneRowTree() {},
		/*
   * 删除多行
   */
		deleteRows: function deleteRows(indexs) {
			var oThis = this,
			    indexss = new Array();
   ...
   * 修改某个cell的值
   */
		updateValueAt: function updateValueAt(rowIndex, field, value, force) {
		...
		updateValueAtTree: function updateValueAtTree(rowIndex, field, value, force) {
			return rowIndex;
		},
		updateValueAtEdit: function updateValueAtEdit(rowIndex, field, value, force) {},
		/*
   * 选中一行
   * slice 设置全选时，slice为true，不做渲染，在setAllRowSelect中统一渲染
   */
		setRowSelect: function setRowSelect(rowIndex, doms) {
   ...
   * 反选一行
   */
		setRowUnselect: function setRowUnselect(rowIndex) {
			var oThis = this;
			if (!this.dataSourceObj.rows[rowIndex]) return true;
			//已经选中退出
   ...
   * 选中所有行
   */
		setAllRowSelect: function setAllRowSelect() {
   ...
   * 反选所有行
   */
		setAllRowUnSelect: function setAllRowUnSelect() {
   ...
   * 获取选中行对应行号
   */
		getSelectRowsIndex: function getSelectRowsIndex() {
			return this.selectRowsIndex;
		},
		/*
   * focus一行
   */
		setRowFocus: function setRowFocus(rowIndex) {
			//已经选中退出
   ...
   * 反focus一行
   */
		setRowUnFocus: function setRowUnFocus(rowIndex) {
			var oThis = this;
			if (!this.dataSourceObj.rows[rowIndex]) return true;
   ...
   * 增加删除时重置数字列
   */
		resetNumCol: function resetNumCol() {
   ...
   * 获取focus行
   */
		getFocusRow: function getFocusRow() {
			return this.focusRow;
		},
		/*
   * 获取focus行对应行号
   */
		getFocusRowIndex: function getFocusRowIndex() {
   ...
   * 根据某个字段值获取rowIndex
   */
		getRowIndexByValue: function getRowIndexByValue(field, value) {
			var index = -1;
			$.each(this.dataSourceObj.rows, function (i) {
				var v = $(this.value).attr(field);
				if (v == value) {
   ...
   * 根据filed设置renderType
   */
		setRenderType: function setRenderType(field, renderType) {
   ...
   */
		...
		setColumnPrecision: function setColumnPrecision(field, precision) {
		...
		setMultiSelect: function setMultiSelect(multiSelect) {
		...
		setShowNumCol: function setShowNumCol(showNumCol) {
		...
		isGridShow: function isGridShow() {
   ...
   * 处理参数
   */
		init: function init(options, gridComp) {
   ...
   * 将values转化为rows并进行排序
   */
		sortRows: function sortRows(field, sortType) {
   ...
   */
		basicSortRows: function basicSortRows(field, sortType) {
			var oThis = this,
			    dataType = "";
			if (field) {
		...
		treeSortRows: function treeSortRows(field, sortType) {
			this.basicSortRows(field, sortType);
		},
		/*
   * 获取合计值
   */
		getSumValue: function getSumValue(field, gridCompColumn, gridComp) {
			var sumValue = null;
