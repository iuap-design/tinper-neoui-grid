const createContentGroupRows = function(createFlag) {
    var htmlStr = '',
        oThis = this;
    if (this.options.groupField && this.dataSourceObj.groupRows && this.dataSourceObj.groupRows.length > 0) {
        $.each(this.dataSourceObj.groupRows, function(i) {
            ...
            if (oThis.options.groupSumRow)
                htmlStr += oThis.createContentGroupSumRow(this, createFlag);
        });
    }
    return htmlStr;
};
    ...
    if (this.options.groupField && this.dataSourceObj.groupRows && this.dataSourceObj.groupRows.length > 0) {
        $.each(this.dataSourceObj.groupRows, function(i) {
            if (this.rows && this.rows.length > 0) {
                ...
                if (oThis.options.groupSumRow)
                    htmlStr += oThis.createContentLetGroupSumRow('multiSelect',this.value);
            }
        });
    }
    return htmlStr;
    ...
    if (this.options.groupField && this.dataSourceObj.groupRows && this.dataSourceObj.groupRows.length > 0) {
        $.each(this.dataSourceObj.groupRows, function(i) {
            if (this.rows && this.rows.length > 0) {
                ...
                if (oThis.options.groupSumRow)
                    htmlStr += oThis.createContentLetGroupSumRow('numCol',this.value);
            }
        });
    }
    return htmlStr;
...
const re_getGroupIndex = function(row, index, rowObj) {
...
const getGroupRowByGroupValue = function(groupValue) {
    var groupRow;
...
const deleteOneRowGroup = function(rowObj) {
    var groupField = this.options.groupField,
